...
Linux is an open-source operating system. Initially conceived by Linus Torvalds in 1991 as the Linux kernel, the platform’s original core has since evolved into its current iterations thanks to global collaboration between thousands of developers and web programmers.
...
3. **Init system:** A sub-system that bootstraps the user space and is charged with controlling daemons.
4. **Daemons:** Background services (printing, sound, scheduling, etc.) that either start up during boot or after you log into the desktop.
5. **General server:** The sub-system that displays the graphics on your monitor. It is commonly referred to as the X server
